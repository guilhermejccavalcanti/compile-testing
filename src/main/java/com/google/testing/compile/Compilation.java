...
final class Compilation {
   ...
   */
    static Result compile(Iterable<? extends Processor> processors, Iterable<? extends JavaFileObject> sources) {
        ...
        null, fileManager, diagnosticCollector, ImmutableSet.<String>of(), ImmutableSet.<String>of(), sources);
        task.setProcessors(processors);
        boolean successful = task.call();
        return new Result(successful, diagnosticCollector.getDiagnostics(), fileManager.getOutputFiles());
    }
   ...
   */
    static Iterable<? extends CompilationUnitTree> parse(Iterable<? extends JavaFileObject> sources) {
        ...
        DiagnosticCollector<JavaFileObject> diagnosticCollector = new DiagnosticCollector<JavaFileObject>();
        InMemoryJavaFileManager fileManager = new InMemoryJavaFileManager(compiler.getStandardFileManager(diagnosticCollector, Locale.getDefault(), UTF_8));
        JavacTask task = ((JavacTool) compiler).getTask(// explicitly use the default because old versions of javac log some output on stderr
        null, fileManager, diagnosticCollector, ImmutableSet.<String>of(), ImmutableSet.<String>of(), sources);
        try {
    ...
    static final class Result {
        ...
        Result(boolean successful, Iterable<Diagnostic<? extends JavaFileObject>> diagnostics, Iterable<JavaFileObject> generatedFiles) {
            this.successful = successful;
            this.diagnosticsByKind = Multimaps.index(diagnostics, new Function<Diagnostic<?>, Diagnostic.Kind>() {
            ...
            this.generatedFilesByKind = Multimaps.index(generatedFiles, new Function<JavaFileObject, JavaFileObject.Kind>() {
        ...
        public String toString() {
            return Objects.toStringHelper(this).add("successful", successful).add("diagnostics", diagnosticsByKind).toString();
        }
    }
}
